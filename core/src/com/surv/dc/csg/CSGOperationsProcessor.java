...
public class CSGOperationsProcessor {
    ...
    private void CSGReduceOperations(Set<ChunkNode> touchedNodes, CSGOperationInfo lastOperation){
        ...
        for (int i=nodes.size()-1; i>-1; i--){
            if(checkNodeForSelection(nodes.get(i), new Vec3f(camera.position.x, camera.position.y, camera.position.z) ) || nodes.get(i).size==meshGen.clipmapLeafSize) {
                activeNodeNumber = i;
                break;
            }
        }
